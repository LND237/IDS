...
use super::ip::IpTrait;
use super::ip::IP;
pub const MAX_PORT: i32 = 65536;
pub const IPPROTO_TCP: u16 = 6;
pub const IPPROTO_UDP: u16 = 17;

pub trait SnifferTrait{
    fn new(ip : IP, port: u16) -> Result<Sniffer, String>;
    fn get_ip(&self) -> IP;
    fn get_port(&self) -> u16;
    
    fn sniff(&mut self, max_amount_packets: i32) -> String;
}
...
impl SnifferTrait for Sniffer{
    ...
    fn get_ip(&self) -> IP {
        return <IP as IpTrait>::copy(&self.ip);
    }
    fn get_port(&self) -> u16 {
        return self.port;
    }
    fn sniff(&mut self, max_amount_packets: i32) -> String{
        ...
        println!("Listening for packets to IP address {}: {}", <IP as IpTrait>::get_ip(&self.ip), interface.name);

        while self.packets.len() < max_amount_packets as usize {
            let mut is_property_packet = false;
            ...
            if is_property_packet{
                println!("Received packet to IP address {}", <IP as IpTrait>::get_ip(&self.ip));
...
fn check_packet_propriety(packet: Vec<u8>, ip: IP, port: u16) -> bool{
    if let Some(ethernet) = EthernetPacket::new(&packet) {
        ...
        if let Some(ipv4) = Ipv4Packet::new(ethernet.payload()) {
            ...
            if ipv4.get_destination().to_string() == <IP as IpTrait>::get_ip(&ip).to_string() {
